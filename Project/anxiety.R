...
# Drawing Graphs
unique(data$age)
data$age <- ifelse( data$age > 100 , round(mean(data$age)) , data$age  )

library(ggplot2)
ggplot(data) + geom_bar(aes(x = age)) + facet_wrap(~depression.lvl)

ggplot(data) + geom_bar(aes(x = gender.fix, fill = depression.lvl))
